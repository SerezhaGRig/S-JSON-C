...
int parse_pair(char* from, int size, char** name, char** val)
{
	...
	int lenght = -1;
	int breck_count = 2;
	for (from++; (*(from) != ':' || breck_count>0) && *from; lenght++, from++)
		if (*from == '\'')
			breck_count--;
	if (!(*from))
		return 1;
	...
	if ((*(from + size) != '\'' || *(from) != '\'') && (*from != '{' || *(from + size) != '}') && (*from != '[' || *(from + size) != ']') && (!isdigit(*from) || !isdigit(*(from + size)))) {
		free(*name);
		return 1;
	}
	if (*from == '\'')
...
char** csarr_from_cstr(char* from,int alloc_size,int* realsize)
{
	...
	for (i=0; *(from - 1) != ']'; from++,i++,lenght = 0) {
		if (*from == ']')
			break;
		if (*from != ',' && *from != '[')
		{
...
char** multiarr_from_cstr(char* from, int alloc_size, int* realsize)
{
	...
	for (lenght = 0; *from != ']'; lenght = 0,i++) {
		...
		from++;
		if (!isdigit(*from) && *from != '\'' && *from != '{' && *from != '[' && *from)
		{
			free_cstr_arr(carr,i);
			return NULL;
		}
		...
		if (!((*start == '\'' && *(from-1)=='\'') || (*start == '{' && *(from - 1) == '}') || (*start == '[' && *(from - 1) == ']') || (isdigit(*start) && isdigit(*(from - 1)))))
		{
			free_cstr_arr(carr, i);
			return NULL;
		}
